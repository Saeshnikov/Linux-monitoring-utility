...
| t:s:sys_enter_ | Описание | Аргументы | Возвращаемые значения |
|:---:| --- | --- | --- |
...
| Link<br>Linkat | Создает новую “жесткую” ссылку на существующий файл. | (int olddfd)<br>Const char* oldname<br>(int newdfd)<br>Const char* newname<br>(int flags)|```?``` Возвращает 0 в случае успеха  |
...
| Readlink<br>Readlinkat| Считывает значение символьной ссылки (помещает содержимое ссылки в path). | (int dfd)<br>Const char* path(name)<br>Char * buf<br>Int bufsiz| ```?``` Открытие самого файла и ссылки как происходит |
...
| statx | Возвращает информацию о файле, записывая ее в буфер. | Int dfd<br>Const char* filename<br>Unsigned flags<br>Unsigned int mask<br>Struct statx * buffer| ```?``` |
| Symlink<br>Symlinkat| Создает новую “символьную” ссылку на существующий файл. | Const char* oldname<br>(int newdfd)<br>Const char* newname | ```?``` Возвращает 0 в случае успеха |
